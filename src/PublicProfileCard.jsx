  ...
  return (
    <div onClick={onClose} className="a360-noprint"
      ...
      <div onClick={(e) => e.stopPropagation()} className="a360-reveal"
        ...
        {/* Banner + chiudi */}
        <div style={{ height: 88, background: `linear-gradient(120deg, ${C.navy} 0%, ${C.navy2} 100%)`, position: "relative" }}>
          <button onClick={onClose} aria-label="Chiudi" style={{ position: "absolute", top: 10, right: 10, background: "rgba(255,255,255,0.15)", border: "none", color: "#fff", borderRadius: 8, cursor: "pointer", padding: 5, display: "inline-flex" }}><X size={18} /></button>
        </div>

        <div style={{ padding: "0 22px 22px", textAlign: "center", marginTop: -46 }}>
          <div style={{ display: "inline-block", background: C.card, borderRadius: "50%", padding: 4 }}>
            <Avatar url={entry?.avatar_url} name={name} size={92} ring={ring} />
          </div>
          <div style={{ ...display, fontSize: 20, fontWeight: 700, color: C.ink, marginTop: 10 }}>{name}</div>
              ...
              {social.map((s, i) => {
                ...
                return (
                  <a key={i} href={s.url} target="_blank" rel="noopener noreferrer" title={s.label}
                    style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 38, height: 38, borderRadius: 10, border: `1px solid ${C.grid}`, background: "#fff", color: C.navy2 }}>
                    <Icon size={18} />
                  </a>
                );
              })}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 18 }}>
            {canMessage && (
              <button onClick={() => onMessage(recipientId, name)}
                style={{ ...font, display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 8, padding: "12px 16px", borderRadius: 12, border: "none", background: C.orange, color: "#fff", fontSize: 14.5, fontWeight: 600, cursor: "pointer" }}>
                <MessageCircle size={17} /> Messaggio privato
              </button>
            )}
            {onFullProfile && (
              <button onClick={() => onFullProfile(identifier)}
                style={{ ...font, display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 8, padding: "11px 16px", borderRadius: 12, border: `1px solid ${C.grid}`, background: "#fff", color: C.navy2, fontSize: 14, fontWeight: 600, cursor: "pointer" }}>
                <ClipboardList size={16} /> Vedi scheda completa
              </button>
            )}
            {isSelf && !onFullProfile && (
              <div style={{ ...font, fontSize: 12.5, color: C.muted }}>Questo è il tuo profilo pubblico.</div>
            )}
          </div>
        </div>
      </div>
