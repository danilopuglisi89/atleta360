...
Meccanismo: un **trigger** sul database invia la mail via **Resend** usando `pg_net`
(chiamate HTTP dal database). Tutto in un solo script:
...
1. Crea un account su [resend.com](https://resend.com) usando **info@danilopuglisi.com**
   (così le email arrivano subito nella tua casella, senza verificare un dominio).
...
> Per ora il mittente è `onboarding@resend.dev` verso la tua email.

### 2. Esegui lo script
1. Apri [`supabase/notify-email.sql`](supabase/notify-email.sql) e sostituisci
